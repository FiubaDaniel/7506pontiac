...
NodoHoja::NodoHoja(std::stringbuf buf,unsigned int cantElem,ComparadorClave* comp) : NodoHoja ::Nodo(0,cantElem,0){
	...
	while(Aux>0){
		...
		ElementoNodoHoja* elemento = new ElementoNodoHoja(ref,clave);
		listaDeElementos.push_back(elemento);
		Aux--;
	}
};
std::list<ElementoNodoHoja*> NodoHoja:: getListaElementos(){
...
void NodoHoja::serializate(stringbuf* buffer){
	...
	std::list<ElementoNodoHoja*>::iterator it;
	it = listaDeElementos.begin();
	if(!listaDeElementos.empty()){
	while(it != listaDeElementos.end()){
		ElementoNodoHoja* elemento = *it;
 ...
 */
int NodoHoja::agregarElemento(ElementoNodoHoja* elemento){

	int retorno = 1;
	if(listaDeElementos.size() == Nodo::getCatidadMaximaDeElementos()){
		retorno = 2;
	}
	if(listaDeElementos.empty()){
		listaDeElementos.push_back(elemento);
	...
	std::list<ElementoNodoHoja*>::iterator it = listaDeElementos.begin();
	bool ubicado = false;
	while(!ubicado || it != listaDeElementos.end()){
		ElementoNodoHoja* elementoAux = *it;
...
void NodoHoja::balanceo(NodoHoja* nodoIncompleto,NodoIntermedio* nodoPadre,bool izq){
	...
	if(izq){
		ElementoNodoHoja * elemento  = listaDeElementos.front();
		...
		listaDeElementos.pop_front();
		elem->setClave(listaDeElementos.front()->getClave()->clonarce());
	}else{
		ElementoNodoHoja * elemento  = listaDeElementos.back();
		std::list<ElementoNodoIntermedio*>:: iterator itAux = nodoPadre->getListaElementos().begin();
		while(!encontrado && itAux != nodoPadre->getListaElementos().end()){
			elem = *itAux;
			if(comparador->Comparar(elemento->getClave(),elem->getClave())<0){
...
ElementoNodoIntermedio* NodoHoja::dividirse(NodoHoja* nodoHermano,NodoHoja* nodoIzq,NodoHoja* nodoMedio,NodoHoja* nodoDer,NodoIntermedio* nodoPadre,Clave clave){
	...
	list<ElementoNodoHoja*>::iterator it = listaDeElementos.begin();
	list<ElementoNodoHoja*>::iterator it2 = nodoHermano->getListaElementos().begin();
 ...
 */
ElementoNodoHoja* NodoHoja::buscar(Clave* clave){
	std::list<ElementoNodoHoja*>::iterator it = listaDeElementos.begin();
	ElementoNodoHoja* elemento;
	bool encontrado = false;
	bool existe = true;
	while(!encontrado && it!=listaDeElementos.end() && existe){
		elemento = *it;
...
Referencia NodoHoja::buscarReferenciaDeClaveX(Clave* clave){
   ElementoNodoHoja* elemento = NodoHoja::buscar(clave);
 ...
 */
int NodoHoja::busquedaSecuencial(Clave* clave,ElementoNodoHoja* elemento){
	std::list<ElementoNodoHoja*>::iterator it = listaDeElementos.begin();
	bool encontrado = false;
	int retorno  = -1;
	while(it != listaDeElementos.end()&& !encontrado){
		elemento = *it;
